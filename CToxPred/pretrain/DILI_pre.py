...
# hERGClassifier 정의
class DILIClassifier(torch.nn.Module):
    def __init__(self, inputSize, outputSize):
        super(DILIClassifier, self).__init__()
...
# 파일 경로 정의
train_file_path = r"C:\Users\ADS_Lab\Desktop\JH\Paper_Replication\CToxPred\dataset\DILI_train_no_salt.txt"
test_file_path = r"C:\Users\ADS_Lab\Desktop\JH\Paper_Replication\CToxPred\dataset\DILI_test_no_salt.txt"
...
model = DILIClassifier(inputSize=inputSize, outputSize=outputSize)
...
best_model_path = "best_DILI_classifier_trained.model"  # 가장 성능 좋은 모델 파일 경로
...
for epoch in range(epochs):
    ...
    with torch.no_grad():
        # 파일명에 에폭 번호 포함
        output_file = f"./DILI/epoch_{epoch + 1}_predictions.txt"
