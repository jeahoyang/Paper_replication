...
# hERGClassifier 정의
class DILIstClassifier(torch.nn.Module):
    def __init__(self, inputSize, outputSize):
        super(DILIstClassifier, self).__init__()
...
# 파일 경로 정의
train_file_path = r"C:\Users\ADS_Lab\Desktop\JH\Paper_Replication\CToxPred\dataset\DILIst_train_no_salt.txt"
test_file_path = r"C:\Users\ADS_Lab\Desktop\JH\Paper_Replication\CToxPred\dataset\DILIst_test_no_salt.txt"
...
model = DILIstClassifier(inputSize=inputSize, outputSize=outputSize)
...
best_model_path = "best_DILIst_classifier_trained.model"  # 가장 성능 좋은 모델 파일 경로
...
for epoch in range(epochs):
    ...
    with torch.no_grad():
        # 파일명에 에폭 번호 포함
        output_file = f"./DILIst/epoch_{epoch + 1}_predictions.txt"
