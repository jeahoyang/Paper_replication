...
def deepod_dsvdd(kcg_intersec, encoded_embedding, EID_to_index, Maybe_cancer_genes_list):
    ...
    # train_genes에서 EID_to_index에 존재하는 값들만 남기기 (추가)
    train_genes = [gene for gene in train_genes if gene in EID_to_index]
    ...
    # remaining_genes에서 EID_to_index에 존재하는 값들만 남기기 (추가)
    remaining_genes = [gene for gene in remaining_genes if gene in EID_to_index]
