...
if __name__ == '__main__':
    ...
    gene_file = "feature_genename_CPDB.txt"
    with open(gene_file, "r") as f:
        # gene_names = [line.strip().split(',')[1] for line in f.readlines()]  # 첫 번째 열만 가져옴
        gene_names = [line.strip().split(',')[0] for line in f.readlines()]  # 첫 번째 열만 가져옴
        # gene_names = gene_names[:12979]
    ...
    for i in range(10):
        ...
        test_mask_file = os.path.join('OncoKB_ONGene_mask\ONGene_CPDB_mask.txt')
        ...
        train_mask = train_mask[:len(test_mask)]
